...
class SearchableSpinner : RelativeLayout {
    ...
    private var customErrorIconPadding: Int = 0
    ...
    private fun prepareTextview() {
        ...
        customErrorIconPadding = resources.getDimensionPixelSize(R.dimen.error_icon_padding)
        val params =
    ...
    fun setError(error: String?){
        ...
        val rightPadding = if (error != null) customErrorIconPadding else 0
        if(mLabelView?.text.toString().trim().isEmpty())
        mLabelView?.setPadding(0, 0, rightPadding, 0)
    }

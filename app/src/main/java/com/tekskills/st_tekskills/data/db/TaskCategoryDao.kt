...
interface TaskCategoryDao {
    ...
    @Transaction
    @Query("SELECT * FROM taskInfo WHERE status = 0 ORDER BY date")
    fun getUncompletedTask(): LiveData<List<TaskCategoryInfo>>

    @Transaction
    @Query("SELECT * FROM taskInfo WHERE status = 1 ORDER BY date")
    fun getCompletedTask(): LiveData<List<TaskCategoryInfo>>

    @Transaction
    @Query("SELECT * FROM taskInfo WHERE status = 0 AND category =:category ORDER BY date")
    fun getUncompletedTaskOfCategory(category: String): LiveData<List<TaskCategoryInfo>>

    @Transaction
    @Query("SELECT * FROM taskInfo WHERE status = 1 AND category = :category ORDER BY date")
    fun getCompletedTaskOfCategory(category: String): LiveData<List<TaskCategoryInfo>>

    @Query(
        "SELECT taskInfo.category as category, Count(*) as count, " +
                "categoryInfo.color as color FROM taskInfo, categoryInfo " +
                "WHERE taskInfo.category == categoryInfo.categoryInformation " +
                "AND taskInfo.status = 0 GROUP BY category " +
    ...
    @Query("SELECT COUNT(*) FROM taskInfo WHERE category = :category ")
    fun getCountOfCategory(category: String): Int

    @Query("SELECT * FROM taskInfo WHERE status = 0 AND date > :currentTime")
